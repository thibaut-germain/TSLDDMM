...
def run_model(data_name, missing_rate, model_name, model_config, EPOCHS=100, SEED=SEED, CHECK=False):
    ...
    out_path = '../results/{}/{}/{}'.format(data_name, str(missing_rate), out_name)
    ...
    if not os.path.exists('../results/{}/{}'.format(data_name, str(missing_rate))):
        os.mkdir('../results/{}/{}'.format(data_name, str(missing_rate)))
    ...
    out_name = '_'.join([data_name, str(missing_rate), model_name, str(SEED)])
    with open('../results/{}/{}/{}'.format(data_name, str(missing_rate), out_name), 'wb') as f:
...
# run experiments
for missing_rate in [0.0, 0.3, 0.5, 0.7][:1]:
    for data_name in valid_datasets[:1]:
        for model_name in model_name_list[:1]:
            try:
                ...
                run_model(data_name, missing_rate, model_name, model_config, EPOCHS=400, SEED=SEED, CHECK=True)
