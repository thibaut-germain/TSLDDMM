...
This choice for $K_\msg$ is motivated by the representation \Cref{theorem:representation} and the following result. 
    ...
    \end{lemma}
    Instead of Gaussian kernels, other types of smooth kernel can be selected as long as the structure \eqref{eq:kernel_TAS} is respected.
    % \begin{proof}
...
Finally, we define the data fidelity loss function, $\scrl$, as a sum of $ d_{\msw^*}^2$ using different kernel's width parameters $\sigma$ to incorporate multiscale information such that. $\scrl$ is indeed differentiable with regards to its first variable.
