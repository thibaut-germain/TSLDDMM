...
\begin{abstract}
    ...
    In this paper, we propose an unsupervised representation learning method (URL) for time series %\cite{trirat2024universal,meng2023unsupervised}
     by giving a special attention to their \textit{shapes} for studying inter-variability in biomedical applications.
    In particular, the method can handle \textit{irregularly sampled} time series having \textit{different sizes}.
    This work belongs to the shape analysis literrature: %which focus on the statistical analysis of various mathematical objects invariant under rotations, dilations, or time parameterization.
     we extend Large Deformation Diffeomorphic Metric Mapping (LDDMM) %\cite{beg2005computing,vaillant2004statistics}
      to the case of time series data by applying deformations to their graph.
    However, the method is not a simple application of LDDMM since time series graph have more structure than 1D curves.
     Indeed, a graph time series transformed by a general diffeomorphism is not always a graph time series.
      We solve this issue by establishing a representation theorem on time series graph and derive its consequences on LDDMM.
